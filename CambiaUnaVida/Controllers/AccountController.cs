        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    if (UserManager.IsInRole(user.Id, "AdminAlbergue"))
                    {
                        return RedirectToAction("Index", "AdminAlbergue");
                    }
                    if (UserManager.IsInRole(user.Id, "Responsable"))
                    {
                        return RedirectToAction("Index", "Responsable");
                    }
                    if (UserManager.IsInRole(user.Id, "Adoptante"))
                    {
                        return RedirectToAction("Index", "Adoptante");
                    }
                    if (UserManager.IsInRole(user.Id, "TrabajadorSocial"))
                    {
                        return RedirectToAction("Index", "TrabajadorSocial");
                    }
                    if (UserManager.IsInRole(user.Id, "Veterinario"))
                    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var result2 = UserManager.AddToRole(user.Id, "Adoptante");

                    return RedirectToAction("Index", "Adoptante"); //HAY QUE REDIRIGIR AL PERFIL DE ADOPTANTE
                }                
